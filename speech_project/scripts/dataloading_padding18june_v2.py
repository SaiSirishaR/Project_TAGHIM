...
eng_data = arctic_database(Train_input)

datass = DataLoader(eng_data, batch_size=4, shuffle=True)

for slt in datass:

    print("slt is", numpy.shape(slt.data))
    sorted =  sorted(slt.data, key=len, reverse=True)
    #slt.data, lengths = sort(slt.data)
    print("aftr sorting", sorted)
    pack = torch.nn.utils.rnn.pack_padded_sequence(autograd.Variable(sorted), lengths, batch_first=True)
