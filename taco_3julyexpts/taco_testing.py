...
for epoch in range(0,100):
 ...
 for ig, (Text, Speech) in enumerate(train_loader):
  if numpy.shape(Text)[0]==16:
###    print("yes shape is:",  numpy.shape(Text)[0])
...
########### Tacotron testing starts here ##########

output_path = '/home3/srallaba/projects/siri_expts/Tacotron/expts_20may2019/Project_TAGHIM/siri_Tacotron_may2019/scripts/SGCM/taco_3july_expts/expt1_wav/'

def generate(model, text):

    # Text to index sequence
    seq = np.expand_dims(np.asarray(text_to_seq(text)), axis=0)

    # Provide [GO] Frame
    mel_input = np.zeros([seq.shape[0], num_mels, 1], dtype=np.float32)

    # Variables
    characters = Variable(torch.from_numpy(seq).type(torch.LongTensor), volatile=True)
    mel_input = Variable(torch.from_numpy(mel_input).type(torch.FloatTensor), volatile=True)
...
sentences = ["This is tacotron voice. How do you like me???"]

# Synthesis and save to wav files
for num, text in enumerate(sentences):
        wav = generate(model, text)
        path = os.path.join(output_path, 'result_%d_%d.wav' % (0,num+1))
        with open(path, 'wb') as f:
            f.write(wav)

        f.close()
        print("save wav file at step %d ..." % (num+1))
