...
###        print("input shape is", numpy.shape(input_))
...
###        print("received prenet info", numpy.shape(prenet), prenet)
        memory = self.cbhg.forward(prenet)
###        print("done with memory", memory)
        return memory
...
class MelDecoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.prenet = prenet(80, hidden_size * 2, hidden_size)
...
#        print("got the encoder output", memory, numpy.shape(memory))
        mel_output = self.decoder1.forward(mel_input, memory)
        print("got mel_output", mel_output, numpy.shape(mel_output))
        print("am going into linear thing", numpy.shape(mel_output))
        linear_output = self.decoder2.forward(mel_output)
#        print("got linear out", linear_output, "shape is", numpy.shape(linear_output))
