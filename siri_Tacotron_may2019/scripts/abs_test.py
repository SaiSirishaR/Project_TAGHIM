# This code is to extract speech features (spectrogram) and reconstruct speech out of the speech features back
...
for file in files:
  ...
  print("specgram shape is", numpy.shape(specgram))
##### Reconstruction from spectrogram ########
  wav = inv_spectrogram(specgram)
  wav = wav[:find_endpoint(wav)]
  wav *= 32767 / max(0.01, np.max(np.abs(wav)))
  librosa.output.write_wav('/home3/srallaba/projects/siri_expts/Tacotron/expts_20may2019/Project_TAGHIM/siri_Tacotron_may2019/scripts/spec_recon_wav/' +file, wav.astype(np.int16), 16000)
