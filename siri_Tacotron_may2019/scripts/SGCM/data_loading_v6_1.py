...
#    magnitude = np.array([spectrogram(w) for w in wave])
    for i in range(0,len(wave)):
      ...
      mag = spectrogram(wave[i])
      print("magnitude initially", numpy.shape(mag))
###    mel = np.array([melspectrogram(w) for w in wave])



##### Reconstruction ########
###    print("mels is", mel, numpy.shape(mel))
      wav = inv_spectrogram(mag)
      wav = wav[:find_endpoint(wav)]
      wav *= 32767 / max(0.01, np.max(np.abs(wav)))
      librosa.output.write_wav('/home3/srallaba/projects/siri_expts/Tacotron/expts_20may2019/Project_TAGHIM/siri_Tacotron_may2019/scripts/SGCM/file_'+str(i+1)+'.wav', wav.astype(np.int16), 16000)
