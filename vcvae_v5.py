...
class arctic_database(Dataset):
   ...
   def __getitem__(self, idx):
    ...
    print("src fils are:", len(src_files))
...
#    tgt_info =  [numpy.loadtxt(tgt_filename) for tgt_filename in tgt_filenames]
    print("loaded source file", src_info)
...
def collate_fn(batch):
    ...
    max_target_len = np.max([len(x) for x in batch]) + 1
    ...
    return input_lengths, src_batch
...
for lengths, slt in datass:

##  print("len of datass is", len(datass))
#  if numpy.shape(slt)[0]==batch_size:
   count += slt.size(0)
#   slt = np.array(slt)
#   bdl = np.array(bdl)
##   print("SF1 is", numpy.shape(slt.data), "Tensor is", numpy.shape(autograd.Variable(torch.Tensor(bdl.data))))
...
#     print((sorted(os.listdir(self.src_dir))[idx]))
    ...
    print("valid files are:", valid_files[idx])
 ...
 ####   print("initial shape", numpy.shape(src_info), "index is", numpy.shape(src_info[idx]))
    return valid_info[idx], valid_filenames
...
for val, fname in val_datass:
   ...
   print("val out shaope is", numpy.shape(val_out), numpy.shape(val_out.detach().numpy()), numpy.shape(val_out.data[0].detach().numpy()), "its name is", fname)
